...
import { MONTHLY_SALES_RATE, FIRMNESS_TYPES } from '../constants';
 ...
 * @example
 * ```ts
 * // King has 150 total springs, sells 30/month
 * calculateCoverage(inventory, 'King')  // Returns 5.0 months
 *
 * // Queen has 0 springs, sells 41/month
 * calculateCoverage(inventory, 'Queen') // Returns 0 months (stockout!)
 * ```
 */
export function calculateCoverage(inventory: Inventory, size: MattressSize): number {
