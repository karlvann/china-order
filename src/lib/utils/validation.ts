...
import type { SpringOrder, ComponentOrder, Inventory } from '../types';
import { MONTHLY_SALES_RATE, COMPONENT_TYPES } from '../constants';

/**
 * Result of equal runway validation
 */
export interface EqualRunwayValidation {
  isValid: boolean;
  size: string;
  componentId: string;
  springCoverage: number;
  componentCoverage: number;
  difference: number;
}

/**
 * Summary of equal runway validation across all sizes and components
 */
export interface EqualRunwayValidationSummary {
  allValid: boolean;
  violations: EqualRunwayValidation[];
  warnings: EqualRunwayValidation[];
}
...
export function validateEqualRunway(
  ...
  componentOrder: ComponentOrder,
  inventory: Inventory,
  thresholdWarning: number = 0.5,
  thresholdViolation: number = 2.0
): EqualRunwayValidationSummary {
  const violations: EqualRunwayValidation[] = [];
  const warnings: EqualRunwayValidation[] = [];
  ...
  const springCoverageCache = new Map<string, number>();
  const componentCoverageCache = new Map<string, number>();
  ...
  sizes.forEach(size => {
    ...
    applicableComponents[size].forEach(componentId => {
      ...
      const validation: EqualRunwayValidation = {
 ...
 */
export function formatValidationMessage(validation: EqualRunwayValidationSummary): string {
 ...
 */
export function validatePalletSize(pallet: any): boolean {
 ...
 */
export function validateAllPallets(springOrder: SpringOrder): boolean {
