...
<template>
  <div class="mb-10">
    ...
    <!-- Loading State -->
    <div v-if="inventoryOrdersStore.loading" class="text-muted text-sm py-4">
      Loading orders...
    </div>
    ...
    <div v-else-if="inventoryOrdersStore.pendingOrders.length === 0" class="bg-surface border border-border rounded-lg p-6 text-center">
      <p class="text-muted text-sm">No pending orders. Create one to track incoming inventory.</p>
    </div>
