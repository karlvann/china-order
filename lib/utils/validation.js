...
import { WEEKLY_SALES_RATE, COMPONENT_TYPES } from '../constants/index.js'

/**
 * Calculate weeks of coverage for springs after ordering
 */
function calculateSpringCoverage(springInventory, springOrder, size) {
  ...
  const weeklySales = WEEKLY_SALES_RATE[size]

  return weeklySales > 0 ? totalStock / weeklySales : Infinity
}

/**
 * Calculate weeks of coverage for a component after ordering
 */
function calculateComponentCoverage(
  ...
  const weeklySales = WEEKLY_SALES_RATE[size]
  const componentSalesRate = weeklySales * component.multiplier
 ...
 * - Difference > 2 weeks = WARNING (King/Queen)
 * - Difference > 4 weeks = WARNING (small sizes - consolidation effects)
 * - Difference > 8 weeks = VIOLATION
 *
 * @param springOrder - Calculated spring order
 * @param componentOrder - Calculated component order
 * @param inventory - Current inventory
 * @param thresholdWarning - Warning threshold in weeks (default 2)
 * @param thresholdViolation - Violation threshold in weeks (default 8)
 */
export function validateEqualRunway(
  ...
  sizes.forEach(size => {
    ...
    applicableComponents[size].forEach(componentId => {
      ...
      const effectiveWarningThreshold = isSmallSize ? 4 : thresholdWarning
