 ...
 *
 * Buckets sales from complete Mon-Sun periods into 6 chunks of 2 weeks (12 weeks total). Each chunk's value
...
export const NUM_WEEKS = LOOKBACK_DAYS / WEEK_DAYS
export const RECENT_SPIKE_WEEKS = 2
export const STORE_SPLIT_WEEKS = 2
...
export function getChunkIndex(saleDate, referenceTime) {
  const diffMs = referenceTime - new Date(saleDate).getTime()
  if (diffMs <= 0) return -1

  const daysAgo = Math.floor((diffMs - 1) / 86400000)
...
export const getCompletedWeekIndex = (saleDate, referenceDate = new Date()) => {
  ...
  const diffMs = currentMonday.getTime() - new Date(saleDate).getTime()
  if (diffMs <= 0) return -1

  const index = Math.floor((diffMs - 1) / (WEEK_DAYS * 86400000))
  if (index < 0 || index >= NUM_WEEKS) return -1
  return index
}

export const emptyWeeks = () => Array.from({ length: NUM_WEEKS }, () => 0)

export const recentWeeklyDemandSpike = (weeklyCounts) => {
  const recentWeeks = weeklyCounts?.slice(0, RECENT_SPIKE_WEEKS) || []
  if (!recentWeeks.length) return 0
  return recentWeeks.reduce((sum, value) => sum + value, 0) / RECENT_SPIKE_WEEKS
}
