...
import { calculateSkuWeeklyDemand } from '../utils/index.js'
...
// Priority weights - higher = more important when urgency is similar
// King/Queen are approximately 78% of sales, so they get slight priority boost
const SIZE_PRIORITY_WEIGHT = {
  King: 1.4,
  Queen: 1.5,
  Double: 1.0,
  'King Single': 1.0,
 ...
 */
export function calculateSkuMetrics(inventory, salesRates, pendingOrders, orderWeekOffset = 0, deliveryWeeks = LEAD_TIME_WEEKS) {
  const skus = []
  ...
  for (const size of ALL_SIZES) {
    pendingSprings[size] = { veryfirm: 0, firm: 0, medium: 0, soft: 0 }
  }
        ...
        if (order.springsByFirmness) {
          ...
          for (const [size, qty] of Object.entries(order.springs)) {
            if (pendingSprings[size]) {
              const firmnessDistribution = salesRates.FIRMNESS_DISTRIBUTION[size] || { veryfirm: 0, firm: 0.33, medium: 0.34, soft: 0.33 }
              for (const firmness of FIRMNESS_TYPES) {
  ...
  for (const size of ALL_SIZES) {
    ...
    for (const firmness of FIRMNESS_TYPES) {
      ...
      const rawSkuWeeklyDemand = salesRates.RAW_SKU_WEEKLY_DEMAND?.[size]?.[firmness] || 0
      const weeklyDemand = calculateSkuWeeklyDemand(sizeWeeklyRate, firmnessRatio, rawSkuWeeklyDemand)
 ...
 */
export function calculateDemandBasedOrder(totalPallets, inventory, salesRates, pendingOrders, orderWeekOffset = 0, deliveryWeeks = LEAD_TIME_WEEKS) {
  // Step 1: Calculate per-SKU metrics
  const skuMetrics = calculateSkuMetrics(inventory, salesRates, pendingOrders, orderWeekOffset, deliveryWeeks)
  ...
  for (const size of ALL_SIZES) {
    ...
    const veryfirmAlloc = springsPerSku[`${size}|veryfirm`] || 0
    ...
    const sizePallets = createPalletsForSize(size, numPallets, { veryfirm: veryfirmAlloc, firm: firmAlloc, medium: mediumAlloc, soft: softAlloc }, palletId)
...
function allocatePalletsToSizes(totalPallets, skuMetrics) {
  ...
  while (palletsRemaining > 0) {
    ...
    for (const size of ALL_SIZES) {
      ...
      // Multiply by priority weight to favor King/Queen when urgency is similar
      const baseUrgency = targetCoverage - minCoverage
      const urgency = baseUrgency * (SIZE_PRIORITY_WEIGHT[size] || 1.0)
  ...
  for (const size of ALL_SIZES) {
    ...
    if (withDemand.length === 0) {
      ...
      const perFirmness = Math.floor(totalSprings / 4)
      springsPerSku[`${size}|veryfirm`] = perFirmness
      springsPerSku[`${size}|firm`] = perFirmness
      springsPerSku[`${size}|medium`] = perFirmness
      springsPerSku[`${size}|soft`] = totalSprings - 3 * perFirmness
      continue
    }
    ...
    const allocation = { veryfirm: 0, firm: 0, medium: 0, soft: 0 }
    ...
    // Assign to output
    springsPerSku[`${size}|veryfirm`] = allocation.veryfirm
    ...
    console.log(`[SPRING DIST] ${size}: veryfirm=${allocation.veryfirm}, firm=${allocation.firm}, medium=${allocation.medium}, soft=${allocation.soft}`)
  }
...
function createPalletsForSize(size, numPallets, firmnessAlloc, startId) {
  ...
  const total = remaining.veryfirm + remaining.firm + remaining.medium + remaining.soft
