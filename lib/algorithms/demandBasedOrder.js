...
function allocatePalletsBySku(totalPallets, skuMetrics) {
  ...
  // Sort by priority: critical sizes first, then by minimum coverage
  sizeNeeds.sort((a, b) => {
    // Critical sizes first
    if (a.hasCritical && !b.hasCritical) return -1
    if (!a.hasCritical && b.hasCritical) return 1

    // Then by minimum coverage (lowest first = most urgent)
    if (a.minCoverage === Infinity && b.minCoverage === Infinity) {
      return 0
    }
    if (a.minCoverage === Infinity) return 1
    if (b.minCoverage === Infinity) return -1
    return a.minCoverage - b.minCoverage
  })
  ...
  // First pass: give each size what it needs (up to available pallets)
  for (const sn of sizeNeeds) {
    if (remaining === 0) break
    if (sn.palletsNeeded === 0) continue

    const toAllocate = Math.min(sn.palletsNeeded, remaining)
    allocation[sn.size] = toAllocate
    remaining -= toAllocate

    console.log(`[SKU ALLOC] ${sn.size}: ${toAllocate} pallets (need ${sn.palletsNeeded}, minCov=${sn.minCoverage.toFixed(1)}wks, critical=${sn.hasCritical})`)
  }

  // Second pass: distribute remaining pallets to sizes with lowest coverage that have room
  while (remaining > 0) {
    const bestSize = findLowestCoverageSize(sizeNeeds, allocation, skuMetrics)
    if (!bestSize) {
      console.log(`[SKU ALLOC] ${remaining} pallets remaining but no sizes can accept more (all overstocked)`)
      break
    }
    allocation[bestSize]++
    remaining--
