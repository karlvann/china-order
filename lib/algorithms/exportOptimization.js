 ...
 *    - Felt only rounds to its lot size; no buffer, so it keeps the 1:3 spring ratio
...
export function optimizeComponentOrder(componentOrder, format) {
  ...
  COMPONENT_TYPES.forEach((comp) => {
    ...
    MATTRESS_SIZES.forEach((size) => {
      ...
      // Felt is a fixed 1:3 spring top-up, so only round to its supplier lot size.
      // Do not add the normal buffer or it breaks the felt-to-spring ratio.
      if (comp.id === 'felt') {
        optimized[comp.id][size.id] = Math.ceil(quantity / comp.lotSize) * comp.lotSize
        return
      }
