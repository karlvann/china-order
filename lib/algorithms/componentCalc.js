...
export function calculateComponentOrder(springOrder, springInventory, componentInventory, salesRates, pendingComponentOrders = [], orderWeekOffset = 0) {
  ...
  // Micro coils and thin latex MUST be EQUALIZED (they're glued together)
  // If stock levels are uneven, order more of the lower one to match the higher
  ...
  // Allow negative projections (backorders) - don't floor at 0
  const projectedMicroKing = currentMicroKing - kingDepletionUntilArrival + pendingMicroKing
  const projectedLatexKing = currentLatexKing - kingDepletionUntilArrival + pendingLatexKing

  // Target = whichever is highest: target coverage, or current stock of either component
  // This ensures we bring the lower one UP to match the higher one (equalization)
  const targetKingLayer = kingLayerDemand * MIN_COVERAGE_TARGET
  const finalKingTarget = Math.max(targetKingLayer, projectedMicroKing, projectedLatexKing)

  // Order each component up to the final target (equalizes stock levels)
  componentOrder['micro_coils']['King'] = Math.max(0, Math.ceil(finalKingTarget - projectedMicroKing))
  componentOrder['thin_latex']['King'] = Math.max(0, Math.ceil(finalKingTarget - projectedLatexKing))
  ...
  // Allow negative projections (backorders) - don't floor at 0
  const projectedMicroQueen = currentMicroQueen - queenDepletionUntilArrival + pendingMicroQueen
  const projectedLatexQueen = currentLatexQueen - queenDepletionUntilArrival + pendingLatexQueen

  // Target = whichever is highest: target coverage, or current stock of either component
  const targetQueenLayer = queenLayerDemand * MIN_COVERAGE_TARGET
  const finalQueenTarget = Math.max(targetQueenLayer, projectedMicroQueen, projectedLatexQueen)

  // Order each component up to the final target (equalizes stock levels)
  componentOrder['micro_coils']['Queen'] = Math.max(0, Math.ceil(finalQueenTarget - projectedMicroQueen))
  componentOrder['thin_latex']['Queen'] = Math.max(0, Math.ceil(finalQueenTarget - projectedLatexQueen))
