 ...
 *
 * Components are ordered to achieve balanced coverage across runout-managed items.
 ...
 * 2. Phase 1: Calculate felt as a fixed 1:3 spring top-up
 * 3. Phase 1-3: Calculate runout-managed component orders (spring-matched + coverage minimum, then balance)
 * 4. Phase 4: Calculate micro/latex TARGET final inventory (they're treated as pairs)
 * 5. Phase 5: Scale runout-managed basic components (felt/micro/latex excluded)
 * 6. Phase 6: Scale and finalize micro/latex orders (ensures they end up EQUAL)
 *
 * Rules:
 * - Felt top-up: 1:3 with springs per size; coverage/runout rules do not apply
 * - Basic panels (top/bottom panels): 1:1 with springs per size
...
export function calculateComponentOrder(springOrder, springInventory, componentInventory, salesRates, pendingComponentOrders = [], orderWeekOffset = 0, deliveryWeeks = 10, palletCount = 12, componentScaleOverride = 1.0) {
  ...
  MATTRESS_SIZES.forEach((size) => {
    springsPerSize[size.id] =
      (springOrder.springs.veryfirm?.[size.id] || 0) +
  ...
  // Track runout-managed components for balancing pass
  const componentTracking = []

  // Felt top-up: springs arrive packaged in usable felt, so only order the extra ~33%.
  // Coverage/runout, balancing, and scaling do not apply to felt.
  MATTRESS_SIZES.forEach((size) => {
    componentOrder['felt'][size.id] = Math.ceil(springsPerSize[size.id] * FELT_TO_SPRING_RATIO)
  })

  // Basic 1:1 components (top_panel, bottom_panel)
  const basicComponents = ['top_panel', 'bottom_panel']
  ...
  if (combinedScale !== 1) {
    COMPONENT_TYPES.forEach((comp) => {
      // Skip felt (fixed spring ratio) plus micro coils/thin latex (handled in Phase 6)
      if (comp.id === 'felt' || comp.id === 'micro_coils' || comp.id === 'thin_latex') return

      MATTRESS_SIZES.forEach((size) => {
        const currentOrder = componentOrder[comp.id][size.id]
        if (currentOrder > 0) {
          const currentStock = componentInventory?.[comp.id]?.[size.id] || 0
          const pendingStock = pendingComponents[comp.id]?.[size.id] || 0
          ...
          if (combinedScale < 1) {
            const minNeeded = calcMinForCoverage(currentStock, weeklyDemand, pendingStock)
