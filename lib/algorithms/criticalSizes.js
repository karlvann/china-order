...
import { SMALL_SIZES, WEEKLY_SALES_RATE, FIRMNESS_TYPES, FIRMNESS_DISTRIBUTION, CRITICAL_THRESHOLD } from '../constants/index.js'

/**
 * Find all critical small sizes (coverage < threshold in weeks).
 *
 * Algorithm:
 * 1. Calculate total and medium coverage for each small size
 * 2. Filter to sizes with total coverage < threshold
 ...
 */
export function findCriticalSmallSizes(inventory) {
  // Calculate coverage for each small size
  const sizesWithCoverage = SMALL_SIZES.map((size) => {
    ...
    const mediumWeeklySales = weeklySales * mediumRatio

    // Calculate coverage for total and medium separately
    const totalCoverage =
      weeklySales === 0 ? (totalStock > 0 ? Infinity : 0) : totalStock / weeklySales
    const mediumCoverage =
      mediumWeeklySales === 0 ? (mediumStock > 0 ? Infinity : 0) : mediumStock / mediumWeeklySales
  ...
  // Filter to only critical sizes (coverage < threshold in weeks)
 ...
 */
export function findCriticalSmallSize(inventory) {
